...
mxt_AICtab <- AICtab(mxt_lm1, mxt_lm2, mxt_lm3, mxt_lm4, mxt_lm5, mxt_null,
                         weights = TRUE, sort = T)
mxt_summ_table <- do.call(rbind, lapply(list(mxt_lm1, mxt_lm2, mxt_lm3, mxt_lm4, mxt_lm5, mxt_null), broom::glance))
...
mxt_AICtab <- AICtab(mxt_lm1, mxt_lm2, mxt_lm3, mxt_null,
                     weights = TRUE, sort = T)
mxt_summ_table <- do.call(rbind, lapply(list(mxt_lm1, mxt_lm2, mxt_lm3, mxt_null), broom::glance))
mxt_AICtab[["Resid.Dev"]] <- mxt_summ_table[["deviance"]]
mxt_AICtab
...
mnt_lm3 <- update(mnt_lm2, ~.-Hmean_log:aspect)

drop1(mnt_lm3, test='Chisq')
mnt_lm4 <- update(mnt_lm3, ~.-LandUse)

drop1(mnt_lm4, test='Chisq')
mnt_lm5 <- update(mnt_lm4, ~.-PAI_log)
...
drop1(mnt_lm2, test='Chisq')
mnt_lm3 <- update(mnt_lm2, ~.-LandUse)
...
#now lets make some AIC tables for model selection
# model table with bbmle/broom package
mnt_AICtab <- AICtab(mnt_lm1, mnt_lm2, mnt_lm3, mnt_lm4, mnt_null,
                     weights = TRUE, sort = T)
mnt_summ_table <- do.call(rbind, lapply(list(mnt_lm1, mnt_lm2, mnt_lm3, mnt_lm4, mnt_null), broom::glance))
mnt_AICtab[["Resid.Dev"]] <- mnt_summ_table[["deviance"]]
mnt_AICtab
...
mxv_lm6 <- update(mxv_lm5, ~.-LandUse)
...
drop1(mxv_lm3, test='Chisq')
mxv_lm4 <- update(mxv_lm3, ~.-LandUse)
...
mnv_lm2 <- update(mnv_lm1, ~.-LandUse)

drop1(mnv_lm2, test='Chisq')
mnv_lm3 <- update(mnv_lm2, ~.-slope)

drop1(mnv_lm3, test='Chisq')
mnv_lm4 <- update(mnv_lm3, ~.-aspect:Hmean_log)

drop1(mnv_lm4, test='Chisq')
mnv_lm5 <- update(mnv_lm4, ~.-aspect)
...
mxt_LandUse <- ggplot(df_lmer, aes(x = LandUse, y = maxTemp)) +
  ...
  scale_fill_manual(values=alpha(c("gray","gray"),0.8)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank()) +
